...
- **Migrations applied through `0023`.** Migrations are pasted into the Supabase SQL editor by the owner (the agent has no DDL access); each is idempotent/re-runnable. Test scripts live in the gitignored `coverage/` dir (node `.mjs`, run against real Supabase, self-cleaning).
