import type { Dictionary } from "@/lib/i18n/types";

export const en: Dictionary = {
  brand: {
    name: "Dijla",
  },

  push: {
    prompt: "Turn on order alerts so you're notified even with the page closed.",
    enable: "Enable",
    enabled: "Alerts on",
    disable: "Turn off",
    denied:
      "Notifications are blocked in the browser — enable them in site settings to receive orders.",
    iosInstall:
      "For alerts on iPhone: Share → “Add to Home Screen”, then open the app from its icon.",
    failed: "Couldn't enable notifications",
  },

  meta: {
    home: "Dijla",
    login: "Sign in | Dijla",
    signup: "Create account | Dijla",
    onboarding: "Set up your restaurant | Dijla",
    orders: "Orders | Dijla",
    menu: "Menu | Dijla",
    settings: "Settings | Dijla",
    offline: "Offline | Dijla",
  },

  areas: {
    najaf: "Najaf",
    kufa: "Kufa",
    mishkhab: "Mishkhab",
    manathira: "Manathira",
    hira: "Hira",
    karbala: "Karbala",
    diwaniya: "Diwaniyah",
    babil: "Babil",
  },

  common: {
    save: "Save",
    cancel: "Cancel",
    delete: "Delete",
    edit: "Edit",
    add: "Add",
    loading: "Loading…",
    saving: "Saving…",
    name: "Name",
    phone: "Phone number",
    area: "Area",
    price: "Price",
    description: "Description",
    optional: "optional",
    image: "Image",
    hidden: "hidden",
    close: "Close",
    language: "Language",
    themeToLight: "Switch to light mode",
    themeToDark: "Switch to dark mode",
    currency: "IQD",
    selectArea: "Select an area",
    phonePlaceholder: "07701234567",
  },

  nav: {
    orders: "Orders",
    menu: "Menu",
    tables: "Tables",
    drivers: "Drivers",
    reports: "Reports",
    settings: "Settings",
    signOut: "Sign out",
    buildingIn: "Coming in {phase}",
    dashboard: "Dashboard",
  },

  phases: {
    task15: "task 1.5",
    task17: "task 1.7",
    phase2: "phase 2",
    phase4: "phase 4",
    phase5: "phase 5",
  },

  home: {
    tagline:
      "Ordering and operations for Iraqi restaurants — a flat monthly fee, never a commission on orders.",
    signIn: "Sign in",
  },

  landing: {
    navFeatures: "Features",
    navPricing: "Pricing",
    madeInNajaf: "Made in Najaf for Iraqi restaurants",
    heroHeadline: "Take your orders directly — with zero commission",
    heroSub: "One system for dine-in, delivery and reports — a flat monthly subscription instead of a cut of every order.",
    ctaTrial: "Try a month free",
    ctaWhatsapp: "Chat on WhatsApp",
    reassure: "No hardware, no app for your customer to install, and same-day activation.",
    calcTitle: "Calculate your savings",
    calcSub: "Enter your numbers and see the difference between commission and a flat fee.",
    calcOrders: "Orders per month",
    calcAvg: "Average order value (IQD)",
    calcYouPayToday: "What you pay delivery apps today",
    calcCommissionNote: "at ~25% commission",
    calcWithDijla: "With Dijla",
    calcPerMonth: "per month",
    calcYouSave: "You save monthly",
    calcYearly: "that's {amount} a year",
    featuresTitle: "Everything you need to run your restaurant",
    featQrTitle: "QR table menu",
    featQrDesc: "Diners scan and order from their table — no app.",
    featDeliveryTitle: "Delivery with your driver",
    featDeliveryDesc: "Send orders to your driver with the map pin and tap-to-call.",
    featDashboardTitle: "Live order dashboard",
    featDashboardDesc: "Track orders live with a per-order timer and sound alert.",
    featReportsTitle: "Reports you understand",
    featReportsDesc: "Your revenue, peak hours, and best-selling items.",
    stepsTitle: "Live the same day",
    step1: "Register your restaurant in five minutes",
    step2: "Build your menu from a template",
    step3: "Print your table QR codes on one sheet",
    pricingTitle: "Simple pricing",
    planBasic: "Basic",
    planPro: "Pro",
    perMonth: "/ month",
    mostPopular: "Most popular",
    noCommission: "No commission on orders — cancel anytime.",
    choosePlan: "Get started",
    finalTitle: "Ready to take orders commission-free?",
    finalSub: "Start today, and go live the same day.",
    feat: {
      qrMenu: "QR table menu",
      liveOrders: "Live order dashboard",
      reports: "Reports & sales",
      onePrice: "Flat price, no commission",
      everythingBasic: "Everything in Basic",
      delivery: "Delivery & driver management",
      drivers: "Driver app & cash collection",
      prioritySupport: "Priority support",
    },
  },

  auth: {
    loginTitle: "Sign in",
    loginSubtitle: "Enter your email and we'll send you a sign-in code.",
    signupTitle: "Create your restaurant account",
    signupSubtitle: "No commission on orders — a flat monthly subscription.",
    email: "Email",
    password: "Password",
    sendCode: "Send sign-in code",
    codeSent: "We sent a code to your email",
    enterCodeFor: "Enter the code sent to",
    verifying: "Verifying…",
    resend: "Resend code",
    resendIn: "Resend in {seconds}s",
    changeEmail: "Change email",
    usePassword: "Use a password instead of a code",
    useCode: "Use an emailed code instead of a password",
    signIn: "Sign in",
    createAccount: "Create account",
    accountCreated: "Account created. Check your email to confirm.",
    noAccount: "No account yet? ",
    haveAccount: "Already have an account? ",
    goSignup: "Create your restaurant",
    goLogin: "Sign in",
    invalidCode: "That code is wrong or has expired.",
    invalidEmail: "That email address is not valid.",
    invalidPhone: "That phone number is not valid. Example: 07701234567",
    loginPasswordSubtitle: "Sign in to manage your restaurant.",
    username: "Username",
    usernamePlaceholder: "username",
    rememberMe: "Remember me",
    forgotPassword: "Forgot password?",
    loginWithCode: "Or sign in with an emailed code",
    showPassword: "Show password",
    hidePassword: "Hide password",
    loginFailed: "That username or password is not correct.",
    codeLoginTitle: "Sign in with a code",
    codeLoginSubtitle: "We'll send a sign-in code to your email.",
    backToPassword: "Sign in with a password",
    forgotTitle: "Reset your password",
    forgotSubtitle: "Enter your email and we'll send a reset code.",
    forgotNote: "We'll send a code to the email registered to the restaurant account.",
    sendResetCode: "Send code",
    resetSent: "If an account exists for that email, we sent a code.",
    newPassword: "New password",
    setNewPassword: "Set password",
    changeCode: "Edit code",
    resetDone: "Your password has been updated.",
    passwordTooShort: "Password must be at least 8 characters.",
    pwWeak: "Weak",
    pwFair: "Fair",
    pwGood: "Good",
    pwStrong: "Strong",
  },

  restaurantTypes: {
    nashami: "Grill house",
    iraqi: "Iraqi",
    fastfood: "Fast food",
    drinks_sweets: "Drinks & sweets",
    other: "Other",
  },

  signup: {
    stepLabel: "Step {n} of 3",
    optional: "optional",
    ownerName: "Owner name",
    ownerNamePlaceholder: "Full name",
    emailVerified: "Email verified",
    usernameChecking: "Checking availability…",
    usernameAvailable: "Username available",
    usernameTaken: "Username taken, try another",
    usernameInvalid: "Latin letters, digits and _ only, 4–20 chars",
    usernameThrottled: "Too many checks, wait a moment and try again",
    restaurantName: "Restaurant name",
    restaurantType: "Restaurant type",
    typeRequired: "Choose a restaurant type.",
    restaurantPhone: "Restaurant phone",
    district: "District",
    districtPlaceholder: "Neighbourhood or area",
    landmark: "Nearest landmark",
    landmarkPlaceholder: "e.g. opposite Al-Kawthar pharmacy",
    pinLabel: "Location pin on the map",
    pinRequired: "Drop the restaurant's pin on the map inside Iraq.",
    locationRequired: "Enter the district and nearest landmark.",
    reviewTitle: "Review & documents",
    reviewHint: "Check the details and attach documents before submitting.",
    reviewFooter: "Reviewed within 24 hours. You can build your menu now.",
    idDocument: "Owner ID",
    idDocumentHint: "Attach an ID photo (image or PDF)",
    licenseDocument: "Shop licence / commercial registration",
    licenseHint: "Attach the licence if available (image or PDF)",
    docTooLarge: "File is too large. The limit is 5 MB.",
    docUploadFailed: "Couldn't upload the documents; you can add them later in settings.",
    createdPending: "Account created — under review. Start building your menu.",
    next: "Next",
    back: "Back",
    submit: "Submit",
    submitting: "Submitting…",
  },

  onboarding: {
    title: "Welcome to Dijla",
    subtitle: "One last step: create your restaurant and start building its menu.",
    signedInAs: "Signed in as",
    restaurantName: "Restaurant name",
    restaurantNamePlaceholder: "Dijla Restaurant",
    slug: "Restaurant link",
    yourName: "Your name",
    yourNamePlaceholder: "Owner's name",
    submit: "Create restaurant",
    created: "Your restaurant is ready",
    logoLater: "You can add a logo from Settings once image upload is enabled.",
    nameTooShort: "That restaurant name is too short.",
    alreadyOnboarded: "This account already belongs to a restaurant.",
    restaurantNotFound: "Restaurant not found.",
  },

  dashboard: {
    ordersTitle: "Orders",
    ordersSubtitle: "Live orders land here as soon as QR ordering is switched on.",
    noOrders: "No orders yet",
    noOrdersHint: "Order intake starts in phase 2 — the table QR menu.",
    yourRestaurant: "Your restaurant",
    yourRestaurantHint: "These details can be edited in Settings.",
    restaurantLink: "Restaurant link",
  },

  menu: {
    title: "Menu",
    subtitle: "The order here is what your customer sees on their phone.",
    addCategory: "Category",
    newCategory: "New category",
    editCategoryName: "Rename category",
    categoryName: "Category name",
    categoryPlaceholder: "e.g. Grills",
    categoryAdded: "Category added",
    categoryRenamed: "Name updated",
    categoryShown: "Category is now visible",
    categoryHiddenToast: "Category hidden",
    categoryDeleted: "Category deleted",
    categoryRequired: "A category name is required.",
    noCategories: "No categories yet",
    noCategoriesHint:
      "Start with a category like “Grills” or “Drinks”, then add items to it.",
    addFirstCategory: "Add your first category",
    importTemplate: "Import a starter menu",
    starterMenuTitle: "Choose a starter menu",
    starterMenuHint: "Start from common items you can edit and delete — far faster than typing from scratch.",
    itemsCount: "{count} items",
    templateImported: "Starter menu imported",
    templateInvalid: "Unknown template.",
    templateFailed: "Couldn't import the template.",
    duplicateItem: "Duplicate item",
    itemDuplicated: "Item duplicated",
    copySuffix: "copy",
    itemNotFound: "Item not found.",
    preview: "Preview menu",
    previewRefresh: "Refresh preview",
    previewNewTab: "Open in a new tab",
    countSummary: "{items} items · {categories} categories",
    readiness: "Menu {pct}% ready",
    addPhotosHint: "add photos to {count} items",
    itemNameSecondary: "Name in a second language",
    itemNameSecondaryPlaceholder: "Persian or English name",
    itemNameEn: "English name",
    itemNameEnPlaceholder: "Iraqi Kebab",
    itemNameFa: "Persian name",
    itemNameFaPlaceholder: "کباب عراقی",
    itemDescriptionEn: "English description",
    itemDescriptionFa: "Persian description",
    translations: "Translations",
    itemTags: "Tags",
    itemPrepMinutes: "Prep time (minutes)",
    itemPrepMinutesPlaceholder: "e.g. 15",
    itemPrepLearned: "Your actual average is {minutes} min — tap to use",
    itemCost: "Cost",
    itemCostHint: "For profit reporting — never shown to customers.",
    permanentSoldOut: "Permanently unavailable",
    permanentSoldOutHint: "Won't auto-restore at the start of the service day",
    pasteLines: "Paste lines",
    pasteLinesHint: "One dish per line, price at the end — e.g. Kebab 12000",
    pasteLinesPlaceholder: "Kebab 12000\nLamb tashreeb 8000\nChicken biryani 10000",
    pastePreview: "{count} ready",
    pasteInvalid: "{count} without a price",
    pasteCreate: "Add {count}",
    pasteCreated: "Added {count} items",
    pasteNoLines: "No valid lines",
    metaSizes: "{count} sizes",
    metaExtras: "{count} extras",
    bulkPhoto: "Bulk photos",
    bulkPhotoHint: "Drop photos here or click to choose — matched by filename",
    bulkPhotoSkip: "Skip",
    bulkPhotoUpload: "Upload {count}",
    bulkPhotoProgress: "{done}/{total}",
    bulkPhotoDone: "Uploaded {count} photos",
    selectItems: "Select",
    selectedCount: "{count} selected",
    markSoldOut: "Mark sold out",
    markAvailable: "Mark available",
    tags: {
      vegetarian: "Vegetarian",
      spicy: "Spicy",
      chef_choice: "Chef's choice",
    },
    optionGroups: "Option groups",
    optionsAfterSave: "Save the item first to add options (size, extras).",
    addGroup: "Add group",
    groupName: "Group name",
    groupNamePlaceholder: "e.g. Size, Extras",
    groupNameRequired: "Group name is required.",
    groupRequired: "Required",
    groupMulti: "Multiple choice",
    deleteGroup: "Delete group",
    optionName: "Option name",
    optionNamePlaceholder: "e.g. Large",
    optionNameRequired: "Option name is required.",
    priceDelta: "Price change",
    addOption: "Add option",
    deleteCategoryTitle: "Delete “{name}”?",
    deleteCategoryWithItems:
      "The {count} items in this category won't be deleted, but they will move to “Uncategorised”. To hide it temporarily instead, use the visibility switch.",
    deleteCategoryEmpty:
      "To hide it temporarily instead, use the visibility switch.",
    moveCategoryUp: "Move category up",
    moveCategoryDown: "Move category down",
    showCategory: "Show category",
    deleteCategory: "Delete category",

    newItem: "New item",
    editItem: "Edit item",
    itemName: "Item name",
    itemNamePlaceholder: "e.g. Iraqi kebab",
    itemPrice: "Price (IQD)",
    itemDescriptionPlaceholder: "Grilled lamb with bread and salad",
    itemImage: "Item photo",
    addItemTo: "Add an item to “{name}”",
    addItemUncategorised: "Add an uncategorised item",
    itemAdded: "Item added",
    itemSaved: "Item saved",
    itemDeleted: "Item deleted",
    itemAvailable: "Item is available",
    itemSoldOut: "Item marked sold out",
    soldOut: "sold out",
    availableForOrder: "Available to order",
    noItems: "No items in this category yet.",
    moveItemUp: "Move up",
    moveItemDown: "Move down",
    deleteItemTitle: "Delete “{name}”?",
    deleteItemBody:
      "Past orders keep their own copy of the name and price and won't change. If it's only temporarily unavailable, use the sold-out switch instead.",
    itemNameRequired: "An item name is required.",
    invalidPrice: "That price is not valid.",
    uncategorised: "Uncategorised",
    uncategorisedHint: "hidden from customers until you move them into a category",
  },

  tables: {
    title: "Tables",
    subtitle: "Each table has its own QR code — a diner scans it and your menu opens.",
    addTable: "Table",
    newTable: "New table",
    editTable: "Edit table",
    number: "Table number",
    numberPlaceholder: "5",
    label: "Description (optional)",
    labelPlaceholder: "By the window",
    noTables: "No tables yet",
    noTablesHint: "Add your tables, then print each one's QR code and put it on the table.",
    addFirstTable: "Add your first table",
    deliveryLink: "Public delivery link",
    deliveryLinkHint: "Share it on WhatsApp or Instagram to take delivery orders.",
    copy: "Copy",
    copied: "Copied",
    copyFailed: "Couldn't copy.",
    added: "Table added",
    saved: "Table saved",
    deleted: "Table deleted",
    activated: "Table is active",
    deactivated: "Table is inactive",
    activeSwitch: "Activate table",
    deleteTitle: "Delete table {number}?",
    deleteBody:
      "The printed QR code for this table stops working once it is deleted. If the table is only temporarily out of service, use the deactivate switch instead.",
    cannotDeleteWithOrders:
      "A table with past orders cannot be deleted. Deactivate it instead so the order history stays intact.",
    numberRequired: "A table number is required.",
    numberTaken: "A table with that number already exists.",
    tokenFailed: "Could not generate a unique code. Try again.",
    regenerate: "Change QR code",
    regenerated: "A new code was generated",
    regenerateTitle: "Change the QR code for table {number}?",
    regenerateBody:
      "The current printed code stops working immediately and you will need to print and stick a new one. Use this if the old code leaked.",
    qrToken: "Table code",
  },

  qr: {
    show: "Show QR code",
    tableTitle: "Table {number}",
    downloadPng: "Download PNG",
    print: "Print",
    printAll: "Print all codes",
    printSheet: "QR code sheet",
    printSheetHint: "{count} tables — ready to print on A4.",
    scanToOrder: "Scan to order",
    restaurantQr: "Restaurant QR",
    poster: "Storefront poster",
    stickers: "Stickers",
    orderOnline: "Order online",
    posterTitle: "Restaurant poster",
    localhostWarning:
      "The current address ({origin}) only works on your own machine. Do not print these codes — set NEXT_PUBLIC_APP_URL to the deployed site address first.",
  },

  customer: {
    tableLabel: "Table {number}",
    invalidTitle: "This code isn't valid",
    invalidBody:
      "The code may be out of date, or the restaurant may be closed right now. Ask a member of staff for another code or for the menu.",
    emptyMenuTitle: "The menu isn't available yet",
    emptyMenuBody: "This restaurant hasn't added any dishes yet. Ask a member of staff to order.",
    addToOrder: "Add to order",
    required: "Required",
    optional: "Optional",
    chooseUpTo: "Choose up to {count}",
    chooseRequired: "Choose the required options",
    quantity: "Quantity",
    increase: "Increase",
    decrease: "Decrease",
    itemNote: "Note (optional)",
    itemNotePlaceholder: "No onions, a little spicy…",
    searchPlaceholder: "Search the menu",
    quickAdd: "Quick add",
    noResults: "No matching items",
    openNow: "Open now",
    closedNow: "Closed now",
    filters: "Filters",
    category: "Category",
    tagsLabel: "Tags",
    priceRange: "Price",
    prepTime: "Prep time",
    priceUnder: "Under {price}",
    priceOver: "Over {price}",
    prepUnder: "Under {count} min",
    showResults: "Show {count}",
    clearFilters: "Clear all",
    hideSoldOut: "Hide sold out",
    soldOutBadge: "Sold out today — back tomorrow",
    added: "Added to your order",
    yourOrder: "Your order",
    items: "items",
    subtotal: "Subtotal",
    reviewOrder: "Review order",
    emptyCart: "You haven't chosen anything yet",
    removeLine: "Remove",
    backToMenu: "Back to menu",
  },

  order: {
    place: "Send order",
    placing: "Sending…",
    placedTitle: "Your order is in",
    orderNumber: "Order number",
    placedBody: "The kitchen has your order. Its status will show here.",
    newOrder: "New order",
    errors: {
      invalid_request: "Something went wrong with the order. Refresh and try again.",
      invalid_table: "This table is no longer available. Ask a member of staff.",
      empty_order: "You haven't chosen anything.",
      too_many_lines: "That order is too large. Send it in two parts.",
      invalid_quantity: "That quantity isn't valid.",
      unavailable_items:
        "One of the dishes is no longer available. Refresh to see the current menu.",
      invalid_options: "One of the options is no longer available. Refresh and re-add the item.",
      closed: "The restaurant is closed and isn't taking orders.",
      not_verified: "This restaurant is under review and isn't taking orders yet.",
      invalid_restaurant: "This restaurant isn't available right now.",
      needs_name: "A name is required.",
      needs_phone: "A phone number is required.",
      needs_location: "Set your location on the map, or write the nearest landmark.",
      delivery_disabled: "This restaurant is not taking delivery orders right now.",
      pickup_disabled: "This restaurant is not taking pickup orders right now.",
      below_min_order: "Your order is under the delivery minimum. Add a little more.",
      rate_limited: "Too many orders in a short time. Wait a moment or ask a member of staff.",
      server_error: "The order couldn't be sent. Try again.",
      network: "No connection. Check your internet and try again.",
    },
  },

  orders: {
    title: "Orders",
    subtitle: "New orders arrive here instantly.",
    board: "Board",
    kitchen: "Kitchen",
    kitchenTitle: "Kitchen screen",
    live: "Live",
    reconnecting: "Reconnecting…",
    soundOn: "Sound on",
    soundOff: "Turn on sound",
    soundBlocked: "The browser blocked audio. Tap again.",
    newBadge: "new",
    unseenCount: "{count} new orders",
    acknowledgeAll: "Seen them all",
    noOrders: "No active orders",
    noOrdersHint: "New orders appear here the moment a table sends one.",
    table: "Table {number}",
    noTable: "No table",
    orderNumber: "Order #{number}",
    justNow: "just now",
    minutesAgo: "{count} min ago",
    hoursAgo: "{count} h ago",
    advanceTo: "Next: {status}",
    cancel: "Cancel order",
    updated: "Status updated",
    movedTo: "Moved to: {status}",
    undo: "Undo",
    reverted: "Reverted",
    cancelTitle: "Cancel order #{number}",
    cancelHint: "Cancelling is final. Give a reason for review and accounting.",
    cancelReason: "Cancellation reason",
    cancelReasonPlaceholder: "e.g. out of stock, customer changed their mind",
    cancelReasonRequired: "Give a reason for the cancellation.",
    keepOrder: "Keep order",
    confirmCancel: "Confirm cancellation",
    cancelled: "Order cancelled",
    laneEmpty: "Nothing here",
    dragBackward: "Can't drag an order backwards",
    shortcuts: {
      title: "Keyboard shortcuts",
      move: "Move between orders",
      advance: "Advance the selected order",
      clear: "Clear selection",
      help: "Show shortcuts",
    },
    doneToday: "Completed today",
    resynced: "Resynced — {count} active orders",
    archive: "Archive",
    archiveTitle: "Today's archive",
    archiveHint: "Orders completed and cancelled today — for accounting and review.",
    archiveEmpty: "No archived orders today.",
    backToBoard: "Back to board",
    completedToday: "Completed",
    revenueToday: "Revenue today",
    cancelledToday: "Cancelled",
    dineIn: "Dine-in",
    updateFailed: "Could not update the status",
    navigate: "Navigate",
    assignDriver: "Assign a driver",
    needsDriver: "Needs a driver",
    driverOffline: "offline",
    assignedDriver: "Assigned driver",
    assigned: "Order assigned to the driver",
    unassigned: "Driver unassigned",
    unassign: "Unassign",
    noDrivers: "No drivers registered",
    noDriversOnline: "No drivers are available right now",
    assignFailed: "Could not assign. Try again.",
    showDone: "Show completed",
    hideDone: "Hide completed",
    status: {
      new: "New",
      accepted: "Accepted",
      preparing: "Preparing",
      ready: "Ready",
      delivered: "Served",
      cancelled: "Cancelled",
      out_for_delivery: "Out for delivery",
    },
  },

  track: {
    steps: {
      new: "Order received",
      accepted: "Accepted by the kitchen",
      preparing: "Being prepared",
      ready: "Ready",
      out_for_delivery: "Out for delivery",
      delivered: "Served",
      cancelled: "Order cancelled",
    },
    byType: {
      delivery: { ready: "Ready for the driver", delivered: "Delivered" },
      pickup: { ready: "Ready to collect", delivered: "Collected" },
    },
    cancelledBody: "Ask a member of staff what happened.",
    readyBody: {
      dine_in: "Your order is ready — it's on its way.",
      delivery: "Your order is ready — a driver will take it out shortly.",
      pickup: "Your order is ready — come and collect it.",
    },
    outForDeliveryBody: "Your order is on its way to you.",
    driverArrived: "Your driver has arrived — collect your order.",
    deliveredBody: {
      dine_in: "Enjoy your meal.",
      delivery: "Your order has arrived. Enjoy your meal.",
      pickup: "Your order has been collected. Enjoy your meal.",
    },
    amountDue: "Amount due",
    payCash: {
      dine_in: "Pay cash when it is served",
      delivery: "Pay cash on delivery",
      pickup: "Pay cash when you collect it",
    },
    callWaiter: "Call a waiter",
    calling: "Sending…",
    waiterCalled: "A waiter has been notified",
    waiterFailed: "Could not send the call",
    waiterRateLimited: "Too many calls. Wait a moment.",
  },
  cash: {
    title: "Cash reconciliation",
    subtitle: "Expected versus collected, per driver — for the end-of-day handover.",
    driver: "Driver",
    delivered: "Delivered",
    expected: "Expected",
    collected: "Collected",
    shortfall: "Shortfall",
    total: "Total",
    noDrivers: "No drivers registered.",
    deliveredNote: "Totals are over {count} delivered delivery orders on this day.",
    prevDay: "Previous day",
    nextDay: "Next day",
  },

  pro: {
    title: "A pro feature",
    body: "Delivery and driver management are part of the paid plan — never a commission on any order. Get in touch to upgrade.",
    trialEnded: "Your trial has ended. Table orders keep running; delivery and drivers return when you upgrade.",
  },

  admin: {
    title: "Admin",
    toDashboard: "To dashboard",
    restaurants: "Restaurants",
    subtitle: "Manage every restaurant on the platform.",
    churn: {
      title: "Restaurants at churn risk",
      quietFor: "Quiet for {days}d",
      warning: "Warning",
      critical: "Critical",
      call: "Call",
      logOutreach: "Log outreach",
      logged: "Outreach logged",
      notePlaceholder: "Call note (optional)",
      save: "Save",
      contactedOn: "Contacted on {date}",
      lifetimeN: "{count} orders total",
    },
    search: "Search by name or link",
    noResults: "No results.",
    ordersN: "{count} orders",
    never: "No activity",
    suspendedTag: "Suspended",
    dormantFilter: "Dormant ({count})",
    dormantTag: "Quiet for {count}d",
    activeSwitch: "Activate restaurant",
    activated: "Restaurant activated",
    suspended: "Restaurant suspended",
    editPlan: "Plan",
    churnReason: "Cancellation reason",
    churnReasonHint: "Why did this restaurant leave?",
    monthlyAmount: "Monthly subscription (IQD)",
    monthlyAmountHint: "Feeds the MRR estimate — the sum of active subscription amounts.",
    totalRestaurants: "Restaurants",
    totalOrders: "Total orders",
    suspendedCount: "Suspended",
    mrr: "MRR",
    planSaved: "Plan saved",
    tier: "Tier",
    basic: "Basic",
    pro: "Pro",
    status: "Status",
    statuses: {
      trial: "Trial",
      active: "Active",
      past_due: "Past due",
      cancelled: "Cancelled",
    },
    daysLeft: "{count} days left",
    startDate: "Start",
    endDate: "End",
    notAllowed: "Not allowed.",
    updateFailed: "Could not update. Try again.",
    verify: "Verify",
    reject: "Reject",
    confirmReject: "Confirm reject",
    rejectReason: "Reason for rejection",
    review: "Review",
    reviewTitle: "Verification review",
    signedUpAgo: "Signed up {count}d ago",
    menuItemsBuilt: "Built {count} menu items",
    ownerNameLabel: "Owner name",
    typeLabel: "Type",
    districtLabel: "District",
    landmarkLabel: "Nearest landmark",
    idDocument: "Owner ID",
    licenseDocument: "Shop licence",
    openDoc: "Open document",
    noDoc: "Not attached",
    verifyDone: "Restaurant verified",
    rejectDone: "Restaurant rejected",
    verificationStatuses: {
      pending: "Under review",
      verified: "Verified",
      rejected: "Rejected",
    },
  },

  verification: {
    pendingTitle: "Your account is under review",
    pendingHint: "You can build your menu now; orders open once approved.",
    rejectedTitle: "Verification was rejected",
    resubmit: "Resubmit for review",
    resubmitted: "Resubmitted for review.",
  },

  start: {
    title: "Let's get started",
    progress: "{done} of {total} steps done",
    menuTitle: "Build your menu",
    menuBody: "Add a category and items with prices — this is what the customer sees.",
    menuCta: "Add items",
    tablesTitle: "Set up your tables",
    tablesBody: "Add a table and print its QR code to place on it.",
    tablesCta: "Add a table",
    readyTitle: "Your restaurant is ready to take orders",
    printCta: "Print QR codes",
  },

  growth: {
    title: "Growth",
    subtitle: "Engagement and retention signals for expansion decisions.",
    newRestaurants: "New restaurants per month",
    showCumulative: "Cumulative",
    showNew: "New",
    cohortTitle: "Cohort retention",
    cohort: "Cohort",
    active7d: "Active (7d)",
    active30d: "Active (30d)",
    ofTotal: "of {total}",
    activation: "Activation rate",
    activatedOf: "{activated} of {total} took a first order",
    ordersPerActive: "Orders per active restaurant",
    perWeek: "per week",
    mrr: "MRR",
    arpu: "ARPU",
    perPaying: "per paying restaurant",
    signups30d: "Signups (30d)",
    churned: "Churned",
    vsPaying: "vs {paying} paying",
    churnLog: "Churn log",
    churnHint: "Who left and why — a number with no reason cannot be fixed.",
    noChurn: "No churn yet.",
    noReason: "No reason recorded",
    externalNote: "CAC and payback are not shown — they need spend data outside the app, tracked by the founder.",
  },

  reports: {
    title: "Reports",
    subtitle: "Your restaurant's sales, top items, and busiest hours.",
    ranges: {
      today: "Today",
      "7d": "7 days",
      "30d": "30 days",
      custom: "Custom",
    },
    from: "From",
    to: "To",
    apply: "Apply",
    revenue: "Revenue",
    orderCount: "Orders",
    avgOrder: "Average order",
    cashCollected: "Cash collected",
    byType: "By order type",
    types: {
      dine_in: "Dine-in",
      delivery: "Delivery",
      pickup: "Pickup",
    },
    topItems: "Top items",
    busiestHours: "Busiest hours",
    ordersN: "{count} orders",
    soldN: "sold {count}",
    noData: "No data in this range.",
    insights: {
      peak: "Your peak is {start}:00–{end}:00 · {pct}% of orders",
      topItem: "{name} is your best seller — {count} orders",
      orderUp: "Orders up {pct}% vs the previous period",
      orderDown: "Orders down {pct}% vs the previous period",
      deliveryShare: "Delivery is {pct}% of your orders",
    },
    profitTitle: "Top earners",
    profitEmpty: "Add item costs in the menu to see profit.",
    profitMissing: "{count} items have no cost — add it to show them here.",
    marginN: "{pct}% margin",
    exportCsv: "Export CSV",
    csvItem: "Item",
    csvQty: "Quantity",
    csvRevenue: "Revenue",
  },

  drivers: {
    title: "Drivers",
    subtitle: "Register your restaurant's drivers so they can take delivery orders on their phones.",
    addDriver: "Add driver",
    driverName: "Driver name",
    driverNamePlaceholder: "The driver's full name",
    phoneHint: "The driver signs in with their phone number and a code you give them — no email, no SMS.",
    register: "Register and issue code",
    empty: "No drivers yet",
    emptyHint: "Add your first driver so they can start taking delivery orders.",
    newCode: "New code",
    activeSwitch: "Activate driver",
    activated: "Driver activated",
    deactivated: "Driver deactivated",
    status: {
      available: "Available",
      busy: "Busy",
      offline: "Offline",
    },
    codeTitle: "Driver sign-in code",
    codeBody: "Give the driver their phone number and this code to sign in at /driver.",
    copyCode: "Copy code",
    codeCopied: "Copied",
    codeOnce: "This code is shown only once. If it is lost, generate a new one.",
    cashToday: "Today's cash",
    cashTodayHint: "What each driver collected on delivery orders completed today.",
    deliveriesCount: "{count} delivered",
    notAllowed: "You are not allowed to manage drivers.",
    needsPro: "Driver management is part of the paid plan.",
    nameTooShort: "That driver name is too short.",
    phoneTaken: "This number could not be registered. It may already belong to a driver.",
    createFailed: "Could not register the driver. Try again.",
    updateFailed: "Could not update the driver. Try again.",
  },

  driverAuth: {
    title: "Driver sign-in",
    subtitle: "Enter your phone number and the code the restaurant gave you.",
    code: "Sign-in code",
    codePlaceholder: "The 8-character code",
    signIn: "Sign in",
    invalidCode: "That phone number or code is wrong.",
    disabled: "Your account is disabled. Contact the restaurant.",
    disabledNotice: "Your account is currently disabled. Contact your restaurant to re-enable it.",
  },

  driverApp: {
    title: "My deliveries",
    noDeliveries: "Nothing assigned to you right now",
    nearestFirst: "Nearest first",
    oldestFirst: "Oldest first",
    kmAway: "≈ {km} km",
    arrived: "Arrived",
    arrivedDone: "Customer notified you arrived",
    noDeliveriesHint: "When a delivery is assigned to you it will appear here immediately.",
    signOut: "Sign out",
    cashInHand: "Cash in hand",
    deliveriesToday: "Today: {count}",
    live: "Live",
    reconnecting: "Reconnecting…",
    soundOn: "Sound on",
    soundOff: "Turn on sound",
    soundBlocked: "Could not start sound. Check your browser settings.",
    newAssignment: "A new delivery was assigned to you",
    itemCount: "{count} items",
    collect: "Collect {amount}",
    paid: "Paid",
    pickedUp: "Picked up",
    pickedUpDone: "On the way",
    markDelivered: "Delivered",
    confirmDelivery: "Confirm delivery",
    amountDue: "Amount due",
    cashOnly: "Cash on delivery",
    cashCollected: "Cash collected",
    cashHint: "Defaults to the order total. Lower it if the customer paid less.",
    confirmDeliveredCollected: "Confirm delivered and cash taken",
    deliveredDone: "Order delivered",
    actionFailed: "Could not complete that. Refresh and try again.",
    invalidCash: "That amount is not valid.",
  },

  waiter: {
    banner: "{count} tables are asking for a waiter",
    table: "Table {number}",
    acknowledge: "Handled",
    acknowledged: "Done",
  },

  trial: {
    daysLeft: "{count} days left in your trial",
    lastDay: "Today is the last day of your trial",
    endingSoon: "Your trial ends soon",
    pastDue: "Your subscription payment is overdue. Features continue for now — please settle it.",
    expiredTitle: "Your trial has ended",
    expiredBody:
      "Thanks for trying Dijla. Get in touch to activate your monthly subscription and keep taking orders.",
    expiredNote:
      "Your menu and QR codes still work — diners can still order, but the dashboard is locked until activation.",
    contact: "Contact us",
    activeTitle: "Subscription active",
  },

  closed: {
    title: "The restaurant is closed right now",
    todayHours: "Today: {open} — {close}",
    closedToday: "Closed today",
    browseOnly: "You can browse the menu, but ordering is only open during opening hours.",
    cannotOrder: "The restaurant is closed and isn't taking orders.",
    unavailableLine: "no longer available",
    removedStale: "An item that is no longer available was removed from your order.",
    ordersOffTitle: "Ordering is not available from this link",
    ordersOffBody:
      "You can browse the menu. To order, call the restaurant or visit in person.",
  },

  location: {
    useMyLocation: "Use my location",
    outsideIraq:
      "The location your browser reported is outside Iraq — usually a VPN or a weak location signal. Move the pin by hand, or write the nearest landmark.",
    mapFailed:
      "The map could not load. Write the nearest landmark and your order will still reach you — a landmark on its own is enough.",
    locating: "Finding you…",
    denied: "Location access was blocked. Place your pin on the map instead.",
    failed: "Could not find your location. Place your pin on the map instead.",
    unsupported: "Your browser can't share a location. Use the map instead.",
    dragHint: "Drag the pin or tap the map to place it exactly.",
    pinSet: "Location set",
    accurate: "Accurate",
    roughAccuracy: "Approximate — drag the pin to correct it",
  },
  checkout: {
    title: "Checkout",
    delivery: "Delivery",
    pickup: "Pickup",
    name: "Name",
    namePlaceholder: "Your name",
    phone: "Phone number",
    location: "Your location",
    landmark: "Nearest landmark",
    landmarkPlaceholder: "Opposite Al-Noor pharmacy, blue building",
    landmarkHelp: "Give the driver something they'll recognise — an address alone isn't enough.",
    notes: "Notes for the driver (optional)",
    notesPlaceholder: "Second floor, call on arrival",
    subtotal: "Subtotal",
    deliveryFee: "Delivery fee",
    total: "Total",
    payCash: "Cash on delivery",
    needLocation: "Set your location on the map, or write the nearest landmark.",
    belowMinOrder: "Your order is under the delivery minimum.",
    minOrderShortfall:
      "The delivery minimum is {minimum}. Add {missing} more to place your order.",
    needName: "A name is required.",
    needPhone: "A phone number is required.",
  },

  settings: {
    title: "Settings",
    subtitle: "Your restaurant details, opening hours and delivery fee.",
    restaurantInfo: "Restaurant details",
    restaurantInfoHint: "The name and logo appear above your menu.",
    logo: "Restaurant logo",
    logoLabel: "logo",
    slugWarning:
      "Changing the link breaks any old link you've shared with customers. Table QR codes are unaffected.",
    slugTaken: "That link is taken by another restaurant. Try a different one.",
    deliverySection: "Delivery and pickup",
    deliverySectionHint: "What a customer can order from your public link.",
    deliveryEnabled: "Take delivery orders",
    deliveryEnabledHint: "The customer drops a pin and the food comes to them.",
    deliveryLocked: "Delivery is part of the paid plan.",
    deliveryProTitle: "Delivery is a pro feature",
    deliveryProBody:
      "Take delivery orders straight from your customers — never a commission on any of them. Get in touch to upgrade.",
    deliveryTrialEnded:
      "Your trial has ended. Table orders keep running, and delivery comes back as soon as you upgrade.",
    deliveryNeedsPro: "Delivery needs the paid plan.",
    pickupEnabledHint: "The customer orders, then collects — no fee, no location.",
    pickupEnabled: "Take pickup orders",
    bothChannelsOff:
      "Both are off. Your link will show the menu, but nobody will be able to place an order from it.",
    deliveryFee: "Delivery fee (IQD)",
    deliveryFeeHint: "Added to delivery orders only — table orders are unaffected.",
    invalidDeliveryFee: "That delivery fee is not valid.",
    minOrder: "Minimum delivery order (IQD)",
    noMinOrder: "No minimum",
    invalidMinOrder: "That minimum is not valid.",
    saved: "Settings saved",

    hours: "Opening hours",
    hoursHint:
      "Shown to customers, and used later to stop taking orders outside opening times.",
    hoursSaved: "Opening hours saved",
    saveHours: "Save opening hours",
    copyToAllDays: "Apply Saturday's hours to every day",
    prepSection: "Order timer",
    prepSectionHint: "After how many minutes an order's timer turns warning, then danger, on the live board.",
    prepWarn: "Warning after (minutes)",
    prepDanger: "Danger after (minutes)",
    prepSaved: "Timer thresholds saved",
    prepInvalid: "Enter valid minutes; danger must be greater than warning.",
    menuLayout: "Menu layout",
    menuLayoutHint: "How your menu looks to customers.",
    layoutSaved: "Menu layout saved",
    layouts: {
      auto: "Automatic",
      grid: "Photo grid",
      list: "Compact list",
      featured: "Featured",
    },
    layoutHints: {
      auto: "Picks the best from your photo coverage.",
      grid: "Two columns, big photos — highest conversion.",
      list: "Compact rows with small thumbnails — for large menus.",
      featured: "A hero item with a big photo, then compact rows.",
    },
    tempClosed: "Temporarily closed",
    tempClosedHint: "Stop taking orders immediately, regardless of opening hours.",
    tempClosedOn: "Orders paused",
    tempClosedOff: "Orders resumed",
    closed: "Closed",
    openAt: "{day} opening time",
    closeAt: "{day} closing time",
    openDay: "Open on {day}",
    crossesMidnight: "runs past midnight",
    incompleteHours: "{day} hours are incomplete.",
    invalidTime: "The time format for {day} is not valid.",
  },

  days: {
    sat: "Saturday",
    sun: "Sunday",
    mon: "Monday",
    tue: "Tuesday",
    wed: "Wednesday",
    thu: "Thursday",
    fri: "Friday",
  },

  upload: {
    add: "Add {label}",
    uploading: "Uploading…",
    change: "Change photo",
    remove: "Remove photo",
    uploaded: "Photo uploaded",
    failed: "Could not upload the photo",
    failedWith: "Could not upload the photo: {error}",
    tooLarge: "The photo is still too large after compression.",
    unsupported: "Unsupported image format. Use JPG, PNG or WebP.",
  },

  pwa: {
    install: "Install the Dijla app",
    installHint: "Open it from your home screen like any other app.",
    installButton: "Install",
    iosHint: "“Add to Home Screen”",
    iosThen: "then",
    iosTap: "Tap",
  },

  offline: {
    title: "No internet connection",
    body: "Check your connection and try again. Orders you already sent are saved.",
    banner: "You are offline",
  },

  errors: {
    notFoundTitle: "Page not found",
    notFoundBody: "The link you opened is wrong or no longer available.",
    errorTitle: "Something went wrong",
    errorBody: "This page could not load. Try again.",
    retry: "Try again",
    home: "Back to home",
  },
};
