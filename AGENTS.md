...
- **Migrations applied through `0025`.** Migrations are pasted into the Supabase SQL editor by the owner (the agent has no DDL access); each is idempotent/re-runnable. Unit tests: `tests/` (Vitest, pure, `npm test`). Integration scripts live in the gitignored `coverage/` dir (node `.mjs`, run against real Supabase, self-cleaning).
